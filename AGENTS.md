...
At runtime both providers are real and required (fail-fast at startup if unconfigured):
- Text: `OpenAICompatibleTextProvider`, configured via `TEXT_PROVIDER_API_KEY` / `TEXT_PROVIDER_BASE_URL` / `TEXT_PROVIDER_MODEL`.
- Image: `OpenAICompatibleImageProvider` (default model `gpt-image-2`), configured via `IMAGE_PROVIDER_API_KEY` / `IMAGE_PROVIDER_BASE_URL` / `IMAGE_PROVIDER_MODEL`. Generated images are written to `STATIC_DIR` (default `data/static`) and served under `/static`.

`MockStructuredTextProvider` and `MockImageProvider` are retained only as test doubles (injected via DI in tests); they are no longer wired into `createApp()`.
