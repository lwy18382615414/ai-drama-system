...
Phase 1 is complete. Phase 2A image-provider task infrastructure is in place, and Phase 2B–2C activate mock character reference images, scene reference images, storyboard first frames, and episode-level batch image generation; later Phase 2 media-generation work is paused.
...
Do not add real media provider calls, queue jobs, or FFmpeg composition work beyond the mock Phase 2A–2C image loop.
